...
origFileTime=$(stat -c %Y "$origFile")
...
if [ -f "$gzippedFile" ]
then
	gzippedFileTime=$(stat -c %Y "$gzippedFile")
else
	gzippedFileTime=1
fi
...
if [ -f "$brotlifiedFile" ]
then
	brotlifiedFileTime=$(stat -c %Y "$brotlifiedFile")
else
...
then
	...
	then
		...
		tmpGzippedFileSize=$(stat -c %s "$tmpGzippedFile")
	...
	then
		...
		tmpBrotlifiedFileSize=$(stat -c %s "$tmpBrotlifiedFile")
		if [ "$tmpBrotlifiedFileSize" -lt "$fileToCompressSize" ]
		then
